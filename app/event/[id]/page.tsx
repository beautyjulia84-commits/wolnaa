      ...
      <div className="max-w-2xl mx-auto px-5 -mt-16 relative z-10 pb-40">
        <div className="mb-8">
          ...
          <div className="flex flex-col gap-3">
            {event.date && <div className="flex flex-col gap-1"><div className="flex items-center gap-3"><span className="w-9 h-9 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-center shrink-0">📅</span><span className="text-zinc-300 text-sm">{formatDate(event.date)}{event.time ? ` · ${event.time} Uhr` : ""}</span></div>{calcCountdown(event.date) && <span className="ml-12 bg-yellow-400 text-black text-xs font-bold px-2 py-0.5 rounded-full w-fit">{calcCountdown(event.date)}</span>}</div>}

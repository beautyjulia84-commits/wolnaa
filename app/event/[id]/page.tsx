...
export default function EventPage() {
  ...
  async function loadEvent() {
    ...
    const found = data.find(e => slugify(e.title) === id);
  ...
  function applyDiscount() {
    ...
    const found = event?.discount_codes?.find(d => d.code.toUpperCase() === code);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="max-w-3xl mx-auto px-4 -mt-20 relative z-10 pb-20">
          ...
          <div className="flex flex-col gap-2">
            {event.date && (
              <div className="flex items-center gap-2.5 text-sm text-zinc-300">
                <span className="w-8 h-8 rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center text-base shrink-0">📅</span>
            ...
            {event.address && (
              <div className="flex items-center gap-2.5 text-sm">
                <span className="w-8 h-8 rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center text-base shrink-0">🗺️</span>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-96 space-y-4">
                  ...
                  {allTickets.map((ticket, i) => (
                    <div key={i} className={`flex items-center justify-between rounded-2xl border px-4 py-3.5 transition-colors ${(ticketQtys[i] ?? 0) > 0 ? "border-yellow-400/50 bg-yellow-400/5" : "border-zinc-800 bg-zinc-950/50"}`}>
                      ...
                      <div className="flex items-center gap-2.5">
                        <button
                          onClick={() => setTicketQtys(p => ({ ...p, [i]: Math.max(0, (p[i] ?? 0) - 1) }))}
                          className="w-8 h-8 rounded-full border border-zinc-700 flex items-center justify-center font-bold hover:border-yellow-400 hover:text-yellow-400 transition-colors text-sm"
                        >−</button>
                        <span className="w-5 text-center font-bold">{ticketQtys[i] ?? 0}</span>
                        <button
                          onClick={() => setTicketQtys(p => ({ ...p, [i]: (p[i] ?? 0) + 1 }))}
                          className="w-8 h-8 rounded-full bg-yellow-400 text-black flex items-center justify-center font-bold hover:bg-yellow-300 transition-colors text-sm"
                        >+</button>
                      </div>
                    </div>
                  ))}
            ...
            {event.discount_codes?.length > 0 && (
              <div className="bg-zinc-900/60 border border-zinc-800 rounded-3xl p-5 backdrop-blur-sm">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <button onClick={applyDiscount} className="rounded-xl bg-yellow-400 text-black font-bold px-4 py-3 text-sm hover:bg-yellow-300 transition-colors shrink-0">
                    Einlösen
                  </button>
                </div>
                {discountSuccess && appliedDiscount && (
                  <p className="text-green-400 text-xs mt-2 flex items-center gap-1.5">
                    <span className="w-4 h-4 rounded-full bg-green-400/20 flex items-center justify-center text-green-400 text-[10px] font-black">✓</span>
                    {appliedDiscount.percent}% Rabatt wird angewendet
                  </p>
                )}
            ...
            {/* Gesamtpreis */}
            {totalTickets > 0 && (
              <div className="bg-zinc-900/60 border border-zinc-800 rounded-3xl px-5 py-4 backdrop-blur-sm flex items-center justify-between">
                <div>
            ...
            {/* Checkboxen */}
            <div className="bg-zinc-900/60 border border-zinc-800 rounded-3xl p-5 backdrop-blur-sm space-y-3">
              ...
              <label className="flex items-start gap-3 cursor-pointer group" onClick={() => setAcceptedLegal(!acceptedLegal)}>
                ...
                <span className="text-zinc-400 text-sm leading-relaxed">
                  Ich akzeptiere <span className="text-yellow-400 underline underline-offset-2">AGB</span> & <span className="text-yellow-400 underline underline-offset-2">Datenschutz</span>.
                </span>
              </label>
            </div>
            ...
            <button
              ...
              disabled={!canProceed}
              className={`w-full rounded-2xl py-5 font-black text-base transition-all duration-200 ${
                canProceed
                  ? "bg-yellow-400 text-black hover:bg-yellow-300 hover:scale-[1.01] active:scale-[0.99] shadow-lg shadow-yellow-400/20"
      ...
      {checkoutOpen && (
        ...
        >
          <div
            className="w-full md:max-w-md bg-zinc-900 border border-zinc-800 rounded-t-[2rem] md:rounded-[2rem] overflow-hidden"
          ...
          >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-zinc-500 text-sm mt-0.5">
                    {totalTickets} Ticket{totalTickets !== 1 ? "s" : ""} · <span className="text-yellow-400 font-bold">{total.toFixed(2)} €</span>
                  </p>
                </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <p className="text-zinc-600 text-xs">Das Ticket wird nach Zahlung an diese E-Mail-Adresse gesendet.</p>
            ...
            <div className="px-6 pb-6 flex gap-3">
              <button onClick={() => setCheckoutOpen(false)} className="flex-1 rounded-xl border border-zinc-700 py-3.5 text-sm font-semibold text-zinc-400 hover:text-white hover:border-zinc-500 transition-colors">
                Zurück
              </button>
              <button
                onClick={submitOrder}
                disabled={loading}
                className="flex-1 rounded-xl bg-yellow-400 text-black font-black py-3.5 text-sm hover:bg-yellow-300 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {loading
                  ? <><div className="w-4 h-4 border-2 border-black border-t-transparent rounded-full animate-spin" /> Laden...</>
                  : "Mit Stripe bezahlen"
                }
              </button>
            </div>

...
export default function PasswortFestlegen() {
  ...
  async function save() {
    ...
    if (error) {
      setLoading(false);
      return setError('Das Passwort konnte nicht gespeichert werden. Bitte fordere einen neuen Link an.');
    }

    if (!email) {
      setLoading(false);
      return setError('Das Konto konnte nicht angemeldet werden. Bitte gehe zur Anmeldung.');
    }

    const loginResponse = await fetch('/api/veranstalter/login', {
      method: 'POST',
    ...
    if (!loginResponse.ok) {
      setLoading(false);
      return setError(loginData.error || 'Das Passwort wurde gespeichert, aber die Anmeldung ist fehlgeschlagen.');
    }
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f9fafb',padding:'16px'}}>
      <div style={{width:'100%',maxWidth:'420px',background:'#fff',borderRadius:'16px',padding:'40px',boxShadow:'0 4px 24px rgba(0,0,0,0.07)',border:'1px solid #f0f0f0'}}>
        ...
        {success ? <p style={{color:'#15803d'}}>Dein Passwort wurde gespeichert. Das Dashboard wird geöffnet…</p> : !ready ? <p style={{color:'#dc2626',fontSize:'14px'}}>Der Link ist ungültig oder abgelaufen. Bitte fordere auf der Loginseite einen neuen Link an.</p> : <>
